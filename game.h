 ...
 * the x-z plane.  There are balls and four paddles in the game board.
 *
 * There is one paddle of length PADDLE_LENGTH on each edge of the square.  Each
 * paddle other than the first, the one on the edge connecting (0, 0) and (1, 0),
 * is controlled by the computer.  
 * 
 * A player is scored on when a ball reaches the player's side, but the paddle isn't 
 * there to block it.  When a player is scored on, he loses a point.  When a player 
 * reaches 0 points, he's eliminated, and the side where his paddle was turns into a wall.  
 * 
 * There are four circles of radius BARRIER_SIZE centered at the corners that limit balls' and paddles'
 ...
 * the direction of a ball after hitting a paddle depends on the place where the
 * ball hit the paddle.
 */
...
//Represents a single paddle
class Paddle {
	private:
		//The maximum speed of the paddle
		float maximumSpeed;
		//The distance from the center of the paddle to the corner on its right
		float pos0;
		//-1 if the paddle is accelerating toward the corner on its right, 1 if it
		...
		//Advances the state of the paddle by a short amount of time
		void step();
	public:
		//Constructs a new paddle with the specified maximum speed
		Paddle(float maximumSpeed1);
		
		//Returns the distance from the center of the paddle to the corner to its
		//right
		float pos();
		//Returns a negative number if the paddle is accelerating toward the
		...
		float speed();
		//Returns the acceleration of the paddle when it is accelerating or
		//decelerating
		float acceleration();
 		//Sets the direction toward which the paddle is accelerating.  A negative
		...
		//Advances the state of the paddle by the specified amount of time
...
class Game {
	private:
		//The four paddles.  The first is the one on the edge connecting (0, 0)
		//and (1, 0), and each subsequent paddle is one edge counterclockwise with
		//respect to the previous one.  A paddle is NULL if it has been eliminated
		...
		//Adjusts the directions in which the computer-controlled paddles are
	...
	public:
		...
		int score(int paddleNum);
		//Advances the state of the game by the specified amount of time
		void advance(float dt);
		//Returns the four paddles.  The first is the one on the edge connecting
		//(0, 0) and (1, 0), and each subsequent paddle is one edge
		//counterclockwise with respect to the previous one.  A paddle is NULL if
